...
class StudentRecruitingOptimizerFromCSV:
    ...
    def set_partner(self, partner_type):
        ...
        if partner_type == 'recruiter':
            ...
            # # 5. リクルーター特有の比較ルールを適用: '学部名称'を'所属'に置き換える
            # if '学部名称' in self.ordered_keys:
            #     idx = self.ordered_keys.index('学部名称')
            #     self.ordered_keys.remove('学部名称')
            #     if '所属' in self.partner_df.columns:
            #         self.ordered_keys.insert(idx, '所属')
            ...
            for i in range(num_students):
                for j in range(num_partners):
                    if pl.value(x[(i, j)]) == 1:
                        ...
                        result = {
                            '学生_応募者コード': student.get('応募者コード', 'N/A'), '学生_カナ氏名': student.get('カナ氏名', 'N/A'),
                            '学生_大学名称': student.get('大学名称', 'N/A'), '学生_学部名称': student.get('学部名称', 'N/A'),
                            '学生_学科名称': student.get('学科名称', 'N/A'), '学生_応募者区分': student.get('応募者区分', 'N/A'),
                            '学生_文理区分': student.get('文理区分', 'N/A'), '学生_性別': student.get('性別', 'N/A'),
                            '学生_性格タイプ': student.get('性格タイプ', 'N/A'), '学生_地域': student.get('地域', 'N/A'),
                            f'{partner_prefix}_社員番号': partner.get('社員番号', 'N/A'), f'{partner_prefix}_カナ氏名': partner.get('カナ氏名', 'N/A'),
                            f'{partner_prefix}_大学名称': partner.get('大学名称', 'N/A'), f'{partner_prefix}_学部名称': partner.get('学部名称', 'N/A'),
                            f'{partner_prefix}_学科名称': partner.get('学科名称', 'N/A'), f'{partner_prefix}_応募者区分': partner.get('応募者区分', 'N/A'),
                            f'{partner_prefix}_文理区分': partner.get('文理区分', 'N/A'), f'{partner_prefix}_性別': partner.get('性別', 'N/A'),
                            f'{partner_prefix}_性格タイプ': partner.get('性格タイプ', 'N/A'), f'{partner_prefix}_地域': partner.get('地域', 'N/A'),
                            f'{partner_prefix}_所属': partner.get('所属', 'N/A'), 'ペナルティ': penalty
                        }
